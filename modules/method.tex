...
\end{equation}
\textcolor{blue}{The Top-$K$ step samples distinct bins without replacement within each dimension, then uses rank alignment to form joint candidates. It is not equivalent to exact Top-$K$ sampling without replacement from the full product distribution; it is a parallelizable structured approximation that avoids enumerating the $V^{|A|}$ joint grid.}
